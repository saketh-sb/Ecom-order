...
public class GlobalExceptionHandler {
    ...
    // Helper to build standard error structure
    ...
    // 400 - Type mismatch (e.g., passing string where int expected)
    ...
    // 405 - Wrong HTTP method used (e.g., POST instead of GET)
    @ExceptionHandler(HttpRequestMethodNotSupportedException.class)
    public ResponseEntity<?> handleMethodNotAllowed(HttpRequestMethodNotSupportedException ex) {
        String message = String.format("HTTP method '%s' not allowed. Allowed methods: %s",
                ex.getMethod(),
    ...
    // 409 - DB constraint violation (optional, if using DB)
